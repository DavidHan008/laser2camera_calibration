% give a approximate intrinsic to guide the program to perform a good corner extraction and reduce your workload
% Rough estimates for principal point and focal length:
fg = [935.94474   938.5178 ]';
cg = [ 942.50075  553.61195]';
kg = [-0.03887 0.02035 -0.0429 0.0235]';
...
if isempty(reposition_corner)
    set(2,'color',[1 1 1]);
    title({['Image ']; ['Next you can reposition badly placed corners.']; ['Press ENTER to continue.']});
    h = get(gca, 'title');
...
%     axis([min_x max_x min_y max_y])
    ...
    while true
        ...
        figure(3);
        %Plot the corners
%         plot( grid_pts(1,:),grid_pts(2,:),'+','color','red','linewidth',2);
        plot(grid_pts(1,:)+1,grid_pts(2,:)+1,'r+');
        %Plot the corresponding number
%         text( grid_pts(1,:)'+3,grid_pts(2,:)'+3,num2str(iNumber') )
%         set(findobj('type', 'text'), 'color', 'red');  

        figure(3);
        title({['Image ']; ['Left click on a corner to replace it.']; ['Right click anywhere to quit replacement mode.']});
        h = get(gca, 'title');
        set(h, 'FontWeight', 'bold')
            
        [xdel,ydel, button] = ginput4(1);
        if button > 1
            break;
        end
        ...
        xdel = xdel*ones( Np,1 );
        ydel = ydel*ones( Np,1 );
        distMatrix = [xdel,ydel] - [grid_pts(1,:)',grid_pts(2,:)']; 
        ...
        if( nearestCornerID == Np )
            grid_pts(1,nearestCornerID) = grid_pts(1,nearestCornerID-1);
            grid_pts(2,nearestCornerID) = grid_pts(2,nearestCornerID-1);
        ...
        colormap(map);
        set(2,'color',[1 1 1]);
        title({['Image ']; ['Left click on the desired new location.']; [' ']});
        h = get(gca, 'title');
        set(h, 'FontWeight', 'bold')
...
%         set(findobj('type', 'text'), 'color', 'red');
        ...
        [xnew,ynew, button] = ginput4(1);%get new loc

        [xynew] = cornerfinder([xnew;ynew],I,winty,wintx);
        xnew = xynew(1);
        ynew = xynew(2);%refine loc with cornerfinder

        grid_pts(1,nearestCornerID) = xnew;
        grid_pts(2,nearestCornerID) = ynew;%save new loc
